...
simple_GD.func <- function(objective_function = function(x) x^2, learning_rate = 0.01, initial_guess_min = -100, initial_guess_max = 100, plot = T,plot_window = 100) {
  ...
  if (plot) {
    ...
    mtext(paste("Learning Rate: ",learning_rate),line = -7.5)
    mtext(paste("Number of epochs: ",length(theta.vec)),line = -8.5)
...
png(filename = "G:/My Drive/Dissertation/figures/gd_lr.png",
...
par(mfrow = c(2,1))
simple_GD.func(plot = T,learning_rate = 0.10);simple_GD.func(plot = T,learning_rate = 0.02)
